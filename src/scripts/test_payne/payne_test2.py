...
def main():
    ...
    parser.add_argument('--neuron-count',
                        dest="neuron_count",
    ...
    for labels_individual_batch_count, test_labels_individual_batch in enumerate(test_labels_individual):
        ...
        if False:
            ...
            labels = np.array([[5000, 4, 0], [5500, 4, 0], [5000, 4, -1.], [5000, 5, 0], [4000, 4, 0]])
            ...
            for j, i in enumerate(labels):
                predict_flux = w_array_2 * sigmoid_def(
                    np.sum(w_array_1 * (sigmoid_def(np.dot(w_array_0, i) + b_array_0)), axis=1) + b_array_1) + b_array_2
                plt.plot(training_spectra.wavelengths, predict_flux, lw=2, c=colors[j])
            plt.show()
